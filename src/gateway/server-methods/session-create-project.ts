  ...
  await workspacePreparations.enqueue(`${storePath}\0${sessionKey}`, async () => {
    ...
    if (pending) {
      ...
      if (baseRef && pending.baseRefPolicy !== "strict") {
        try {
          if (pending.baseRefPolicy === "validate" && project) {
        ...
        if (baseRef) {
          // Preserve an accepted ref across later setup failures and retries.
          await persistPending({ ...pending, baseRefPolicy: "strict" });
        }
      }
