...
describe("OpenAI realtime voice bridge connection", () => {
  ...
  it.each([false, true])(
    "sends one shared GA policy and retires despite a throwing close callback (%s)",
    async (throwingCloseCallback) => {
      ...
      await expect(
        provider.createBrowserSession?.({
          ...
          vadThreshold: 0.7,
          voice: "marin",
          gatewayControl: { bindBridge, onEvent, onReady, onClose },
        }),
