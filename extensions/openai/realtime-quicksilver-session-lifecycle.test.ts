...
async function reserveRetirementSession(
  ...
  ownerConnId?: string,
  gatewayControl?: RealtimeVoiceBrowserSessionCreateRequest["gatewayControl"],
) {
  const reservation = await realtime.broker.createBrowserSession(
    {
      providerConfig: {},
...
describe("GA Realtime call retirement", () => {
  ...
  it.each([
    { scope: "global", count: 8, ownerConnId: undefined },
    { scope: "per-client", count: 2, ownerConnId: "conn-retiring" },
  ])(
    ...
    async ({ count, ownerConnId }) => {
      ...
      try {
        ...
        await expect(
          reserveRetirementSession(replacement, () => retirementBridge(), ownerConnId),
